...
def process_single_image(image_prediction, video_name, frame_num):
    ...
    with open(f"static/files/pose_data_{video_name}.txt", "a") as f:
...
def track_skier(video_name, skier_number, start_frame):
    ...
    with open(f"static/files/pose_data_{video_name}.txt", "r") as f:
        data = f.read().split("\n\n")
        all_poses = []
        all_boxes = []
        for frame in data[1:]:
    ...
    with open(f"static/files/skier_tracked_{video_name}.txt", "w") as dataset_f:
...
def get_tracked_skier(video_name):
    ...
    """
    with open(f"static/files/skier_tracked_{video_name}.txt", "r") as f:
        data = f.read().split("\n\n")
        poses = []
        boxes = []
        for frame in data:
